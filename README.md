...
P0 工程基础与 P1 启动链已完成，P2 内核基础与中断正在实施。当前镜像能从自写 Stage 1/Stage 2 经 A20、E820 和保护模式，使用 ATA PIO 加载 ELF32 高半内核；内核校验 Boot Info、建立 4 KiB 早期页表、跳入高半入口、清零 `.bss`，并初始化 COM1/VGA、最小格式化输出、panic 基础与正式 Ring 0 GDT。P2 后续将完成 IDT、异常、PIC/PIT 和键盘。真实状态以 `docs/progress.md` 和阶段报告为准。
