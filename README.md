...
M0 工程基础已完成；T10 的 512-byte BIOS Boot Sector 与 T11 的 Stage 2 实模式框架均已通过真实 QEMU 验收。启动链能够完成 `S1 → S2` 交接，Stage 2 会建立独立栈、保存启动盘号、输出串口诊断，并提供可复用的 BIOS 字符输出和 EDD 读盘接口。A20、E820、保护模式与内核加载将在 T12–T15 继续实现。真实状态以 `docs/progress.md` 和任务报告为准。
...
- docs/task-reports/T03-qemu-test-framework.md：T03 的串口协议、QEMU/GDB 入口、进程清理与测试证据。
- docs/task-reports/T10-boot-sector.md：T10 的 BIOS Boot Sector、磁盘读取、交接合同与测试证据。
- docs/task-reports/T11-stage2-real-mode.md：T11 的实模式运行时、BIOS 接口与真实 QEMU 测试证据。
