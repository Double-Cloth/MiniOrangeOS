...
P0 工程基础、P1 启动链、P2 内核基础/中断、P3 内存管理与 P4 进程/系统调用均已完成验收。P4 已交付 Ring 3/TSS、静态 PCB、独立内核栈、三线程 PIT 抢占、用户地址空间、用户故障隔离、单 CPU 可抢占安全 Heap，以及 `int 0x80` 的 `getpid/write/yield/sleep/exit/waitpid/getticks`。下一阶段是 P5 ELF 用户态与 Shell；真实状态以 `docs/progress.md` 和阶段报告为准。
