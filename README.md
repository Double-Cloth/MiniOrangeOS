...
P0 工程基础、P1 启动链、P2 内核基础/中断、P3 内存管理、P4 进程/系统调用与 P5 ELF 用户态/Shell 均已完成验收。系统现可严格加载静态 ELF32 用户程序，由 `/bin/init` 拉起 Shell，通过共享 ABI 执行 read/write/spawn/wait/ps 等调用，并运行 echo、ps、memtest、fault。下一阶段是 P6 磁盘、MiniFS 与 VFS；真实状态以 `docs/progress.md` 和阶段报告为准。
