...
MiniOrangeOS 是一个从零实现的 x86 32 位 BIOS 教学操作系统。项目包含自写的两级启动链、高半 ELF32 内核、分页与内存管理、Ring 3 用户态、抢占式调度、`int 0x80` 系统调用、ATA PIO、可写 MiniFS、Shell、16 个静态用户程序，以及宿主/QEMU/CI 自动化测试。
...
- 仓库必须位于 WSL 可通过 `/mnt/<drive>` 访问的 Windows 本地盘；仓库目录本身不再固定。
...
确认输出只指向 `MiniOrangeOS-Dev` 和 `${AuthorizedRoot}\rootfs` 后，执行精确确认：
