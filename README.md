...
P0 工程基础、P1 启动链、P2 内核基础/中断与 P3 内存管理已完成，P4 进程与系统调用正在实施。当前已安装 Ring 3 GDT/TSS，并完成静态 PCB、独立内核栈与 PIT 时间片抢占的 round-robin 内核线程调度；Ring 3 进入、用户故障终止和 `int 0x80` 仍待完成。真实状态以 `docs/progress.md` 和阶段报告为准。
