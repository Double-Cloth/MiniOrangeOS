...
MiniOrangeOS 是一个 x86 32 位 BIOS 演示操作系统。项目包含两级启动链、高半 ELF32 内核、分页与内存管理、Ring 3 用户态、抢占式调度、`int 0x80` 系统调用、ATA PIO、可写 MiniFS、Shell、16 个静态用户程序，以及宿主/QEMU/CI 自动化测试。
...
- `/bin/init`、`sh`、`echo`、`ps`、`memtest`、`fault`、`ls`、`cat`、`touch`、`write`、`mkdir`、`rm`、`cp`、`stat`、`sleep`、`uptime`；
- 全量构建、测试、双启动持久化演示、代码量统计和 Linux CI。

项目未实现实现 x86_64、UEFI、SMP、网络、USB、图形桌面、动态链接、Swap、完整 POSIX、权限系统和文件系统日志。MiniFS 不保证任意掉电点的事务原子性；ATA 仅支持 primary master LBA28 PIO。
...
- WSL 可通过 `/mnt/<drive>` 访问的 Windows 本地盘；仓库目录本身无需固定位置。

`environment/wsl/create.ps1` 会从脚本位置找到仓库根目录，并自动推导对应的 WSL 路径。WSL 发行版、下载缓存和备份仍需使用绝对路径以维持安全边界，该路径集中配置在 `config/wsl.psd1` 的 `AuthorizedRoot`；迁移环境时只需修改这一处并重新创建发行版。
...
| `cat` | 顺序读取一个或多个文件 | `cat first second` |
| `touch` | 创建一个或多个空文件 | `touch first second` |
| `write` | 创建或覆盖文件；`-n` 省略末尾换行 | `write hello "persistent data"` |
