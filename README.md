...
P0 工程基础、P1 启动链、P2 内核基础/中断与 P3 内存管理已完成，P4 进程与系统调用正在实施。当前已安装 Ring 3 GDT/TSS，完成静态 PCB、独立内核栈、PIT 时间片抢占、独立用户页目录切换、用户故障隔离及 `int 0x80` 的 `getpid/write/yield/exit` 最小闭环；sleep/waitpid 和 PID 耗尽复用等基础进程生命周期仍待完成。真实状态以 `docs/progress.md` 和阶段报告为准。
