...
P0 工程基础、P1 启动链、P2 内核基础/中断与 P3 内存管理已完成，P4 进程与系统调用正在收口。当前已完成 Ring 3/TSS、静态 PCB、独立内核栈、PIT 抢占、用户地址空间、用户故障隔离、可抢占安全 Heap，以及 `int 0x80` 的 `getpid/write/yield/sleep/exit/waitpid/getticks`；P4 实现范围已闭合，正在执行最终回归与阶段合并。真实状态以 `docs/progress.md` 和阶段报告为准。
