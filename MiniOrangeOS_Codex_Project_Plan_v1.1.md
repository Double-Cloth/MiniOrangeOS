...
> 制定日期：2026-07-13  
> 目标平台：x86 32 位  
> 日常开发环境：独立 WSL2 Ubuntu 24.04 发行版 `MiniOrangeOS-Dev`  
> 真实 Linux 复验环境：Ubuntu 24.04 主机上的 rootless OCI 容器  
> Windows 角色：仅提供 WSL2、终端和编辑器界面；不安装任何原生编译、调试或虚拟化工具链  
...
4. 所有源码编辑、Git 操作、编译、QEMU 运行、GDB 调试和测试均在独立 WSL2 发行版或真实 Ubuntu 的隔离容器中完成；Windows 仅作为启动器和界面宿主。
...
- Windows 上不保留第二份可被 Windows Git 修改的工作树；
...
```powershell
$EnvRoot = "D:\IsolatedEnvs\MiniOrangeOS"
```

该目录应包含：

```text
D:\IsolatedEnvs\MiniOrangeOS\
...
powershell -ExecutionPolicy Bypass -File .\environment\wsl\create.ps1 `
  -InstallRoot "D:\IsolatedEnvs\MiniOrangeOS"
```
...
6. 将项目仓库克隆到 `/home/minios/work/MiniOrangeOS`；
...
```powershell
powershell -ExecutionPolicy Bypass -File .\environment\wsl\enter.ps1 `
  -Command "cd ~/work/MiniOrangeOS && ./environment/with-env.sh make test"
```
...
wsl.exe -d MiniOrangeOS-Dev -- bash -lc `
  'cd ~/work/MiniOrangeOS && ./environment/with-env.sh make test'
```

工作树、`.git`、构建目录和磁盘镜像均保存在 WSL ext4 文件系统中。Windows 可以通过 `\\wsl.localhost\MiniOrangeOS-Dev\home\minios\work\MiniOrangeOS` 查看文件，但禁止使用 Windows Git、Windows Make 或 Windows 编译器操作该目录。
...
powershell -ExecutionPolicy Bypass -File .\environment\wsl\backup.ps1 `
  -Output "D:\IsolatedEnvs\MiniOrangeOS\exports\MiniOrangeOS-Dev.tar"
```
...
1. `MiniOrangeOS-Dev` 是日常开发的唯一权威工作树；Windows 不维护第二份活动工作副本。
2. Codex 即使运行在 Windows 界面中，也必须通过 WSL 入口执行所有文件修改、Git、构建、测试和调试命令。
3. 所有源码、Makefile 和脚本以 Linux 为唯一运行目标；PowerShell 仅用于创建、进入、备份和销毁 WSL 环境。
4. `.gitattributes` 强制以下文件使用 LF：
   - `.c`、`.h`、`.asm`、`.ld`；
   - `Makefile`；
   - `.sh`、`.py`、`.yml`；
   - `Containerfile`、`.env`、`.txt` 版本清单。
5. 禁止 Windows 原生 Git 与 WSL Git 同时操作同一个工作树；禁止从 NTFS 路径 `/mnt/c`、`/mnt/d` 进行高频构建。
6. Codex 每个任务开始时必须执行：
...
7. 每个任务结束时必须在 WSL 中运行指定测试，并由 Linux CI 再验证一次。
...
```text
你正在实现 MiniOrangeOS。必须先阅读 MiniOrangeOS_Codex_Project_Plan_v1.1.md、
docs/README.md、docs/development-workflow.md，以及本任务涉及模块的文档。

执行规则：
1. 确认当前命令运行在 `MiniOrangeOS-Dev` WSL、真实 Ubuntu 隔离容器或 Linux CI 中；禁止在 Windows 原生 Shell 中直接操作工作树。
2. 运行 `./environment/verify.sh` 和 `git status --short`，环境验证失败时立即停止。
...
11. 创建 Git commit，但不要自动合并 main。
12. 最后报告：环境指纹、设计、修改文件、命令、测试结果、风险和提交哈希。
13. 未在 WSL 或真实 Ubuntu 容器中通过且未经 Linux CI 验证的任务不得标记为完成。
...
- 文本文件行尾为 LF；
- `build/`、镜像和工具链不进入 Git；
- README 明确项目目标、专用 WSL 工作方式和真实 Ubuntu 容器复验方式；
...
Windows 只允许使用专用 WSL2 发行版 MiniOrangeOS-Dev，不安装任何 Windows 原生开发工具；
真实 Ubuntu 使用 rootless Podman 或已有 Docker 的项目容器。
...
日常开发必须在独立 WSL2 Ubuntu 24.04 发行版 MiniOrangeOS-Dev 中进行。
即使你的界面运行在 Windows，也只能通过 wsl.exe 在该发行版内修改文件、运行 Git、
编译、启动 QEMU 和使用 GDB。Windows 不安装 GCC、NASM、Make、QEMU、GDB，
...
- 在 Windows 安装原生开发工具或用 Windows Git 操作 WSL 工作树；
- 修改 Windows PATH、Linux 全局 Shell 配置、宿主 /usr/local；
- 使用无范围的容器 prune 或删除其他 WSL/容器资源；
- 在未通过 Linux CI 时声称完成。

现在只执行 T00：
1. 确认当前位于 MiniOrangeOS-Dev WSL，并运行 environment/verify.sh（脚本尚未实现时记录为 T00 待办）；
2. 检查当前仓库；
