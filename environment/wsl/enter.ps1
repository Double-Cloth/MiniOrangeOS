...
function Invoke-WslShellCommand {
    param([string]$Executable, [string]$Name, [AllowEmptyString()][string]$ShellCommand)
    if ([IO.Path]::GetExtension($Executable) -ieq '.cmd') {
        # 测试替身；生产 wsl.exe 与精确 argv 测试均走 ProcessStartInfo。
        & $Executable -d $Name -- bash -lc $ShellCommand
    ...
    # wsl.exe 的 legacy option parser 不接受被引号包裹的 -d/--；发行版名已经
    # 被限制为安全单段，只有任意 shell 字符串需要 CommandLineToArgvW 编码。
    $Info.Arguments = '-d ' + $Name + ' -- bash -lc ' +
