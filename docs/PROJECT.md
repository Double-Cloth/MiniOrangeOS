...
Shell 提供 256-byte 行缓冲、8 项命令历史、退格/Delete/方向键/Home/End 行编辑、`Ctrl+A/E/C/D/K/L/U` 控制、最多 16 项 argv、单/双引号、反斜杠转义、`/bin/` 命令路径解析、当前目录提示符、前台 spawn/wait 和 `shutdown` 主动关机。首个命令词支持 `Tab` 补全：候选由内建命令与实时遍历的 `/bin` 普通文件共同组成，唯一候选追加空格，多候选按最长公共前缀扩展，无法继续扩展时逐行列出候选并恢复提示符、输入内容和光标位置。每个进程保存独立工作目录，子进程继承父进程目录；内核在进入 VFS 前统一规范化绝对/相对路径、重复 `/`、`.` 与 `..`。
