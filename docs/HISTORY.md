...
## 2026-07-18 Shell Tab 命令补全

目标：在不扩展 syscall ABI、不引入依赖的前提下，为交互 Shell 增加首个命令词的 `Tab` 自动补全。开发分支为 `feature/shell-tab-completion`。

- 候选集合由 6 个内建命令与实时遍历的 `/bin` 普通文件共同组成，新增用户程序无需同步维护外部命令硬编码表；
- 唯一候选补齐剩余字符并追加空格；多个候选先扩展最长公共前缀，无法继续扩展时逐行列出匹配项，再恢复当前目录提示符、完整输入行和光标位置；
- 空输入、参数位置、命令词中间和超过 255-byte 行容量的补全安全保持原输入，不改变既有引号、转义、历史与控制键语义；
- Ring 3 Shell 自检覆盖内建、外部和歧义候选收集，真实 PS/2/QEMU 验收覆盖 `ec<Tab>`、`he<Tab>` 与 `c<Tab>` 的执行和候选显示。

验证：

- Windows 独立 Python 运行时完成构建合同与项目布局专项 25/25 PASS；Clang 以 i686 freestanding、项目同等级 `-Werror` 规则完成 `sh.c` 语法编译；
- 正式 `MiniOrangeOS-Dev` 聚合入口 `./environment/with-env.sh make test` 完成环境验证、`i686-elf-gcc` 编译链接、镜像 fsck 和 253/253 宿主/QEMU 测试 PASS，测试用时 659.754 秒，完整命令用时 665.4 秒。

本轮未引入外部代码或新依赖，未修改 MiniFS 磁盘格式、系统调用编号或命令解析 ABI。补全范围明确限定为首个无引号命令词，不扩展为参数、文件路径、变量或完整 POSIX Shell 补全。
...
## 2026-08-20 实现与文档协同优化

目标：在不改变公开 ABI 的前提下，优化内核热点路径，并同步修正文档准确性与课程交付可读性。

本轮实现优化：

- `kernel/mm/usercopy.c`：`copy_user_string` 从“逐字节重复校验”调整为“按页分块校验 + 分块内逐字节扫描”，减少长字符串路径的重复页表查询；
- `kernel/fs/vfs.c`：`vfs_readdir` 的目录名填充改为“两段式拷贝与清零”，移除循环内条件分支并保留长度钳制。

本轮文档优化：

- PROJECT 文档同步 usercopy 最新实现语义；
- DEVELOPMENT 文档补充文档维护规范；

验证说明：

- 上述实现改动完成后，相关文件静态错误检查通过；
- 宿主测试在本机曾遇到 WSL BasePath 一致性问题，需先修复环境注册路径后再执行完整回归。
