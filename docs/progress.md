...
| P6 磁盘与 MiniFS | 进行中 | 已完成 ATA/block、磁盘 ABI、mkfs/fsck/装配、可写 MiniFS、VFS/file object/进程 fd，以及 open/read/write/lseek/close/create/stat；全部用户 ELF 从磁盘加载，退出自动清理 fd；临时镜像双启动持久化与 fsck PASS；启动专项 33/33、MiniFS 工具 6/6、构建契约 10/10；目录扩容、mkdir/unlink/readdir 与用户命令待完成 |
