...
| P4 进程与系统调用 | 进行中 | Ring 3 GDT/TSS、静态 PCB/16 KiB 内核栈、PIT 抢占、用户页目录切换及 `int 0x80` 的 `getpid/write/yield/exit` 已通过真实 QEMU；启动专项 28/28、上一稳定点全量宿主 221/221；用户故障终止、sleep/waitpid 与 PID 复用待完成 |
