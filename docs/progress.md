...
| P4 进程与系统调用 | 进行中 | Ring 3/TSS、静态 PCB/16 KiB 内核栈、PIT 抢占、用户页目录、用户 #PF 隔离、sleep/waitpid、PID 耗尽复用及 7 个最小 `int 0x80` 调用已通过真实 QEMU；启动专项 28/28、上一稳定点全量宿主 222/222；Heap 并发边界和阶段最终回归待完成 |
