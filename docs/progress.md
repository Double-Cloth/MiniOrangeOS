...
| P4 进程与系统调用 | 进行中 | Ring 3 GDT/TSS、静态 PCB/16 KiB 内核栈、PIT 抢占、用户页目录切换、用户 #PF 隔离及 `int 0x80` 的 `getpid/write/yield/exit` 已通过真实 QEMU；启动专项 28/28、全量宿主 222/222；sleep/waitpid、PID 复用与并发边界待完成 |
