...
当前 P1 增量已经在上述实模式运行时之上完成 A20 验证、E820 原始条目采集、临时 GDT 和保护模式切换；ATA 读取、Boot Info 和内核 ELF 加载仍未完成。
...
Stage 2 使用 BIOS E820 获取内存布局。当前原始缓冲固定为 `0x00018000`，每项 24 字节，最多保留 128 项；零长度、ACPI 扩展属性无效的条目被忽略，地址加长度溢出会失败停机。完整路径必须处理：
