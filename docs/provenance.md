...
| GDT/IDT/TSS | 从零实现 | Intel 手册 | 否 | Loader/P2 GDT、256 项 IDT 与异常入口已验收；P4 Ring 3 code/data 描述符、available 32-bit TSS、`ltr` 和动态 `esp0` 已通过真实 QEMU |
...
| PMM/VMM/Heap | 从零实现 | E820 接口、x86 页表机制与分配算法概念 | 否 | P3 PMM/VMM/Heap、用户页目录与 usercopy 已通过自检；P4 主内核 PDE/CR3 与单 CPU IRQ-safe Heap 已通过真实 QEMU；真实 kernel/user #PF 均已验收 |
| Scheduler | 从零实现 | x86 cdecl、中断栈与调度算法概念 | 否 | P4 PCB、独立内核栈、汇编上下文切换、PIT round-robin、sleep/waitpid、PID 复用与用户 #PF 隔离已通过真实 QEMU 自检 |
| Syscall | 从零实现 | x86 中断机制资料 | 否 | P4 DPL3 `int 0x80` 入口、P5 进程调用及 P6 `open/read/write/lseek/close/stat/create/mkdir/unlink/readdir` 文件调用已通过真实 Ring 3 正常/负面路径自检 |
| ELF Loader | 从零实现 ELF32 Header/Program Header 校验、分段读取、BSS 清零和入口翻译 | ELF 规范 | 否 | P1 Kernel ELF Loader 与 P5 用户 ELF Loader均已通过成功路径及 header/范围/内核边界负面测试 |
| ATA PIO | 从零实现有界轮询的 primary master LBA28 读取与写入 | ATA 资料、QEMU 行为说明 | 否 | P1 Loader 读取 Kernel ELF；P6 内核驱动完成 IDENTIFY、多扇区 I/O、容量边界、错误/超时、cache flush 与关中断串行化，并通过真实 QEMU 验收 |
| MiniFS | 自主设计 CRC32 Superblock、bitmap、inode、目录和直接块/一级间接块格式 | 文件系统教材概念 | 否 | P6 确定性 mkfs/fsck、内核挂载、路径解析、可写文件/目录、VFS/fd 及双启动持久化均已通过宿主与真实 QEMU 验收 |
| User libc | 从零实现最低封装 | C ABI 概念 | 否 | P5 crt0、进程 syscall wrapper 与最小字符串函数，以及 P6 文件 syscall wrapper 和 ls/cat/touch/write/mkdir/rm 已由真实磁盘用户程序验收 |
| Test harness | 自主实现严格串口协议、QEMU/GDB 编排、超时与进程树清理 | QEMU/GDB 命令接口 | 否 | T03 独立审查和真实 WSL/QEMU/GDB 回归已验收 |
| Linux CI | 自主编排固定容器构建、只读源码挂载、完整测试入口和失败证据导出 | GitHub Actions workflow、Docker CLI；`actions/checkout` v6.0.2、`actions/upload-artifact` v7.0.1 | 是，仅两个固定提交的 GitHub 官方 action 与固定 OCI 基础镜像 | P7 workflow 将 checkout 固定为 `de0fac2e4500dabe0009e67214ff5f5447ce83dd`、失败上传固定为 `043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`，权限限制为 `contents: read`；最终分支 HEAD `72add84` 的原生 Ubuntu 24.04 运行 `29331275773` 完成环境验证及 246/246 PASS，成功路径未生成失败 artifact |
