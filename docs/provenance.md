...
| Syscall | 从零实现 | x86 中断机制资料 | 否 | P4 DPL3 `int 0x80` 入口及 P5 `read/write/spawn/waitpid/ps` 等共享 ABI 已通过真实 Ring 3 正常/负面路径自检 |
...
| User libc | 从零实现最低封装 | C ABI 概念 | 否 | P5 crt0、syscall wrapper 与最小字符串函数已由 init/Shell/基础命令真实运行验收 |
