...
| Linux CI | 自主编排固定容器构建、只读源码挂载、完整测试入口和失败证据导出 | GitHub Actions workflow、Docker CLI；`actions/checkout` v6.0.2、`actions/upload-artifact` v7.0.1 | 是，仅两个固定提交的 GitHub 官方 action 与固定 OCI 基础镜像 | P7 workflow 将 checkout 固定为 `de0fac2e4500dabe0009e67214ff5f5447ce83dd`、失败上传固定为 `043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`，权限限制为 `contents: read`；本地合同已验收，原生 Linux runner 首次运行待远端触发 |
