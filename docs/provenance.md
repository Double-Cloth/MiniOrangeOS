...
| PMM/VMM/Heap | 从零实现 | E820 接口、x86 页表机制与分配算法概念 | 否 | P3 PMM/VMM/Heap、用户页目录与 usercopy 已通过资源回收、映射冲突、跨页、权限和越界自检；P4 主内核 PDE 刷新、CR3 激活/恢复与权限收紧已通过真实 QEMU；真实 kernel #PF 已验收 |
| Scheduler | 从零实现 | x86 cdecl、中断栈与调度算法概念 | 否 | P4 PCB、独立内核栈、汇编上下文切换、PIT round-robin、sleep/waitpid、PID 复用与用户 #PF 隔离已通过真实 QEMU 自检 |
| Syscall | 从零实现 | x86 中断机制资料 | 否 | P4 DPL3 `int 0x80` 入口及 `exit/write/getpid/yield` 已通过真实 Ring 3 正常/负面路径自检；其余调用待后续阶段 |
