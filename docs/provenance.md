...
| Linux CI | 自主编排固定容器构建、只读源码挂载和完整测试入口 | GitHub Actions workflow、Docker CLI；`actions/checkout` v4.2.2 | 是，仅官方 checkout action 与固定 OCI 基础镜像 | P7 workflow 将 checkout 固定为提交 `11bd71901bbe5b1630ceea73d27597364c9af683`、权限限制为 `contents: read`；本地合同已验收，原生 Linux runner 首次运行待远端触发 |
