...
| Paging | 从零实现 | Intel 手册 | 否 | P2 早期页目录、4 KiB 页表、恒等/高半双映射和 CR3/CR0 切换已通过真实 QEMU；正式 VMM 待 P3 |
| Console/panic | 从零实现 | 16550 UART、VGA text mode 与 C varargs 接口 | 否 | P2 COM1/VGA 双输出、最小格式化和 panic 停机路径已构建；正式镜像格式化日志通过真实 QEMU |
| PIC/PIT/keyboard | 从零实现 | 8259、8254/PIT、PS/2 控制器接口 | 否 | P2 PIC 重映射、IRQ0-15 入口、EOI 与 100 Hz PIT tick 已通过真实 QEMU；键盘待完成 |
