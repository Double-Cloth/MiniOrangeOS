...
当前可写内核接口为 `minifs_create`、`minifs_write` 和 `minifs_truncate`。bitmap 分配器跳过保留区与 inode 0；新数据块先清零，数据与 indirect 索引成功落盘后才提交 inode `size`。写入不允许产生稀疏文件，截断当前只支持缩小；截断会释放多余 direct/indirect 数据块，并在不再需要时释放 indirect 索引块。单次 I/O 失败会尽力回滚本次新分配 bit、指针或目录项；最低版本没有日志，宿主断电级原子性不在当前保证范围内。

创建普通文件已支持向现有目录数据块的空闲尾部追加目录项。目录块扩容、`mkdir`、`unlink` 与完整 VFS/fd 语义仍属于后续 P6 增量。`KERNEL_TEST_MINIFS_WRITE=1` 只用于临时测试镜像：首次启动创建并写入跨 direct/indirect 边界的 `/p6-persist`，再次启动校验内容并缩小文件；默认产品构建不会执行该写入自测。
