...
当前内核挂载位置由 `config/image-layout.json` 生成 C 头，不在内核中重复硬编码。挂载会核对设备容量、完整 Superblock CRC32、元数据几何、元数据 block bitmap、root inode bitmap 和 root 目录；所有 inode、direct/indirect 指针、目录项和文件内容均显式 little-endian 解码。路径解析已支持 `/`、重复 `/`、`.`、`..`、尾随 `/` 和多级目录。确定性镜像现导入 12 个用户 ELF，其中 P5 过渡注册表保留的 6 个基础程序仍在真实 QEMU 中逐字节比对；新增文件命令只从磁盘加载，不扩展过渡注册表。
...
P5 为在 MiniFS 实现前验证正式 ELF loader、用户 ABI 与 Shell，允许把 6 个基础用户 ELF 作为只读构建产物链入内核注册表。P6 当前已由 VFS 读取 `/bin/init` 及其全部子程序 ELF；注册表只保留为过渡期逐字节磁盘一致性自测来源，不再参与 `spawn(path, argv)` 的运行时路径解析，也不得成为第二套可变文件系统。`ls/cat/touch/write/mkdir/rm` 不进入注册表，因此它们的真实执行同时证明磁盘 `/bin` 是唯一运行时来源。
