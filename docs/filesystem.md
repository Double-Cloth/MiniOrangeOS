...
当前内核挂载位置由 `config/image-layout.json` 生成 C 头，不在内核中重复硬编码。挂载会核对设备容量、完整 Superblock CRC32、元数据几何、元数据 block bitmap、root inode bitmap 和 root 目录；所有 inode、direct/indirect 指针、目录项和文件内容均显式 little-endian 解码。只读路径解析已支持 `/`、重复 `/`、`.`、`..`、尾随 `/` 和多级目录，并在真实 QEMU 中把磁盘 `/bin` 的 6 个 ELF 与 P5 过渡注册表逐字节比对。可变分配和修改语义在后续增量接入。
