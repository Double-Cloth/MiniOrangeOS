...
当前 P3 提供非当前地址空间的创建、用户页映射/查询/解除映射和销毁 API。新页目录复制当前稳定的高半内核 PDE，并以 PDE 1023 建立自身递归映射；`0xD0800000-0xD0801FFF` 两页仅作为构造期间的物理页工作窗口，不会复制到目标页目录。用户映射强制位于 `0xC0000000` 以下并设置 U/S，重复映射与高半越界被拒绝；销毁时回收全部用户物理页、低端页表和页目录。

P3 API 只负责离线构造和资源所有权，尚不切换 CR3。P4 在引入进程生命周期时负责激活页目录，并在激活前刷新后续新增的共享内核 PDE；传给地址空间映射 API 的物理页必须是调用者独占的 PMM 分配页，成功映射后所有权转移给该地址空间。
