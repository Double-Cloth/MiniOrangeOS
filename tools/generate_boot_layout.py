#!/usr/bin/env python3
"""从可信镜像布局原子生成 Stage 1/Stage 2 使用的 NASM 常量。"""
...
def _load_boot_components(raw: bytes) -> tuple[int, int, int, int]:
    try:
    ...
    kernel = kernel_components[0]
    kernel_lba = _integer(kernel["lba"], "kernel.lba")
    kernel_sector_count = _integer(
        kernel["max_sectors"], "kernel.max_sectors"
    )
    if not 0 <= kernel_lba <= MAX_ATA_LBA:
        _fail("kernel.lba 超出 ATA PIO LBA28 范围")
    if kernel_lba + kernel_sector_count > MAX_ATA_LBA + 1:
        _fail("kernel 区域越过 ATA PIO LBA28 边界")
    return lba, sector_count, kernel_lba, kernel_sector_count
...
def _render(
    ...
    return (
        ...
        f"%define KERNEL_LBA 0x{kernel_lba:08X}\n"
        f"%define KERNEL_MAX_SECTORS {kernel_sector_count}\n"
    ).encode("ascii")
...
def main(arguments: list[str] | None = None) -> int:
    ...
    try:
        ...
        with BoundTree.open(repo, options.build_dir) as tree:
            ...
            values = _load_boot_components(raw)
            tree.write_output(options.output, _render(*values))
